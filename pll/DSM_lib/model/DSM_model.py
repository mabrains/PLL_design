...
Period1 = [i for i in range(2,Yout1_len//2+1) if Yout1_mod[:i]*(Yout1_len//i)==Yout1_mod[:Yout1_len - Yout1_len % i]]
Period2 = [i for i in range(2,Yout2_len//2+1) if Yout2_mod[:i]*(Yout2_len//i)==Yout2_mod[:Yout2_len - Yout2_len % i]]
...
plt.semilogx(Freq, np.fft.fftshift(SignalFreq1_dB) - max(SignalFreq1_dB), label="1st Order")
plt.semilogx(Freq, np.fft.fftshift(SignalFreq2_dB) - max(SignalFreq2_dB), label="2nd Order")
plt.semilogx(Freq, np.fft.fftshift(SignalFreq3_dB) - max(SignalFreq3_dB), label="3rd Order")
