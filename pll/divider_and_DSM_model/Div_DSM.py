...
Fraction_Internal = 3355443

#for Fraction_Internal in range (0,Accumulator_Size):                      # Choosing channel aka choosing the fraction to divide by
...
Div_in = [0] * len(DSM_out)
Div = [0] * len(DSM_out)
for i in range(0,len(DSM_out)):
	Div_in[i] = 240 + DSM_out[i]
	print(Div_in[i])
	'''
	if DSM_out[i] < 0:
...
for i in range(0,len(DSM_out)):
	Div_out[i] = Divider[Div[i]]
print(np.mean(Div_out))
exit()
