...
freqResolution = freq_array[1]-freq_array[0]
...
pts_num = 3
window_size = (pts_num-1)*freqResolution
phaseNoise = moving_average(mag_array,pts_num) - 10*np.log10(window_size) - mag_array[0]
offset = np.arange(0,phaseNoise.shape[0])*freqResolution +(window_size/2)
...
plt.title("divider phase noise",fontsize=14)
plt.show()
'''
print(mag_array)
print("*************")
