...
freq_file = pd.read_csv ("../csv_files/vout_fft.csv",index_col=False,usecols=[0],header=None, delimiter=r"\s+")
freq_array = freq_file.to_numpy()
freq_array = freq_array.ravel()
freq_array = np.array(freq_array)

mag_file = pd.read_csv ("../csv_files/vout_fft.csv",index_col=False,usecols=[1],header=None, delimiter=r"\s+")
...
plt.title("Common Source Phase Noise @ 1 GHZ With 2 KHZ Frequency Offset Step",fontsize=14)
plt.annotate("(1,-638)", (0.94,-634), fontsize=16)
plt.annotate("(2,-646)", (1.94,-642), fontsize=16)
plt.annotate("(3,-626)", (3,-622), fontsize=16)
plt.show()
