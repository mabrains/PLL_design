...
# --------------------------MABRAINS LLC------------------
#
# This script dets inside the raw file and converts
# the array of a spesific variable measurement from
# binary type into decimal type and
# Then multiple operations on some intended arrays
# are made such as plotting, calculating fft and so on
#
#########################################################
...
def freq_meas(x_axis, y_axis, thresh, edge_type):
    # This function measures the frequencies values of an array
    arr__temp = np.sign(np.subtract(y_axis, thresh * np.ones(len(y_axis))))
    if (
        ((1 not in np.sign(np.diff(y_axis))) and edge_type == "rise")
    ...
    if edge_type == "rise":
        ...
        rise_idx = np.where(
            ((arr_up == 1) & (arr_dn == -1)) | ((arr_up == 1) & (arr_dn == 0))
        )[0]
    ...
    t_cross = np.add(
        t_dn, np.divide(np.subtract(thresh * np.ones(len(y_dn)), y_dn), slope)
    )
...
def t_meas(x_axis, y_axis, thresh, occur_num, edge_type):
    # This function measures the TRIG and TARG of an array
    arr__temp = np.sign(np.subtract(y_axis, thresh * np.ones(len(y_axis))))
    if (
        ((1 not in np.sign(np.diff(y_axis))) and edge_type == "rise")
        or ((-1 not in np.sign(np.diff(y_axis))) and edge_type == "fall")
